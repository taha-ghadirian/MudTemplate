...
pulumi config set MudTemplate.Iac:sql-server-td-db-admin-id
pulumi config set MudTemplate.Iac:sql-server-td-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set MudTemplate.Iac:default-email-from
pulumi config set MudTemplate.Iac:email-server-host
pulumi config set MudTemplate.Iac:email-server-port
pulumi config set MudTemplate.Iac:email-server-userName
pulumi config set MudTemplate.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set MudTemplate.Iac:identity-certificate-password --secret
```
